...
| glean.error.preinit_tasks_timeout |[boolean](https://mozilla.github.io/glean/book/user/metrics/boolean.html) |Set to true if the tasks that are queued prior to Glean initialization time out. |[1](https://bugzilla.mozilla.org/show_bug.cgi?id=1575848#c5)||never |
| glean.validation.app_forceclosed_count |[counter](https://mozilla.github.io/glean/book/user/metrics/counter.html) |This metric counts how many times a dirty bit was detected when starting the Glean SDK, on Android. This dirty bit is set to `true` when Android's lifecycle `ON_START` is hit and flipped back to false on `ON_STOP`. When an application is force-closed, it might not get the `ON_STOP` and so the value would stay `true`. The next time Glean starts, it will increment this counter. |[1](https://bugzilla.mozilla.org/show_bug.cgi?id=1600340#c3)||2019-12-31 |
| glean.validation.baseline_ping_count |[counter](https://mozilla.github.io/glean/book/user/metrics/counter.html) |The number of baseline pings sent during the lifetime of this metrics ping. |[1](https://bugzilla.mozilla.org/show_bug.cgi?id=1597980#c3)||2019-12-20 |
