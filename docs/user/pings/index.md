...
- **Storage quota**: Pending pings are stored on disk. The pings directory is scanned every time Glean is initialized and upon scanning Glean checks how many ping files it contains. If this directory exceeds 250 pending ping files, files are deleted to get the directory back to an accepted size. Ping files are deleted oldest first, until the directory size is below the quota.
