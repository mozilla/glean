...
| `app_display_version` | String | The user-visible version string (e.g. "1.0.3"). The meaning of the string (eg. whether semver or a git hash) is application-specific. In the unlikely event this value can not be obtained from the OS, it is set to "inaccessible". If it is accessible, but not set by the application, it is set to "Unknown". |
