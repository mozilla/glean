...
- **Storage quota**: pings are stored on the `pending_pings` directory. That directory is scanned every time Glean is initialized and upon scanning Glean checks its size. If this directory exceeds a quota of 10MB, pings are deleted to get the directory back to an accepted size. The deleted pings are always the oldest pings in the directory and the number of deleted pings is the necessary to get the directory back to an accepted size.
