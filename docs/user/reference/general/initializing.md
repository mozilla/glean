...
| `collectionEnabled` | _Required_ | The user preference on whether or not data collection and upload is enabled. |
...
>
> Calling `Glean.setCollectionEnabled(false)` at a later point will trigger [`deletion-request` pings](../../user/pings/deletion-request.md) and remove the client ID afterwards.
> Calling `Glean.setCollectionEnabled(true)` will generate a new client ID.
