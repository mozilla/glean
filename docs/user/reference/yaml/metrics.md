...
>
> This does not apply to Firefox Desktop, where all metrics for a component must be declared in a
> file called _exactly_ `metrics.yaml`.
...
Declaring the schema at the top of a metrics definitions file is advised,
...
Metrics files generated by `glean_parser` will be split by category and each file will be named
after the category it contains metrics for. See the ["Capitalization"](../../user/metrics/adding-new-metrics.md#capitalization)
note to understand how the category is formatted before being used as a file name.
