...
## Adding new definition files in the future

If you are adding new definition files (`metrics.yaml` or `pings.yaml`) to your project at a later stage,
you will need to add these files to the corresponding entry in probe scraper.
Add the path to `metrics_files` or `ping_files` for your library or application and open a new Pull Request.

If you expect these additions to happen more frequently, please reach out to us on [#glean:mozilla.org](https://chat.mozilla.org/#/room/#glean:mozilla.org)
and we can assist in automating that step.
