...
internal fun parseISOTimeStringAsCalendar(date: String): Calendar? {
    ...
    for (format in DATE_FORMAT_PATTERN_VALUES) {
        val dateFormat = SimpleDateFormat(format, Locale.US)
        try {
            // Note: since we're parsing from ISO8601 (strings that are generated by
            // `getISOTimeString`) we need to set the "UTC" timezone. If we don't do that,
            // DST might be applied again every time we parse.
