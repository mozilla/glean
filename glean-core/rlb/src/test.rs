...
fn send_a_ping() {
    ...
    let custom_ping = private::PingType::new(PING_NAME, true, true, true, true, true, vec![]);
...
fn send_a_ping_without_info_sections() {
    ...
    let custom_ping = private::PingType::new(PING_NAME, true, true, true, false, true, vec![]);
    custom_ping.submit(None);
...
fn ping_collection_must_happen_after_concurrently_scheduled_metrics_recordings() {
    ...
    let ping = private::PingType::new(ping_name, true, false, true, true, true, vec![]);
...
fn flipping_upload_enabled_respects_order_of_events() {
    ...
    let sample_ping = PingType::new("sample-ping-1", true, false, true, true, true, vec![]);
    let metric = private::StringMetric::new(CommonMetricData {
        name: "string_metric".into(),
        category: "telemetry".into(),
...
fn registering_pings_before_init_must_work() {
    ...
    let sample_ping = PingType::new("pre-register", true, true, true, true, true, vec![]);
...
fn test_a_ping_before_submission() {
    ...
    let sample_ping = PingType::new("custom1", true, true, true, true, true, vec![]);
