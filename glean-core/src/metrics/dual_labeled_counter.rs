...
pub fn validate_dynamic_key_and_or_category(
    ...
    if let Some((mut key, mut category)) = separate_label_into_key_and_category(&label) {
        ...
        let (key_count, category_count) = count_keys_and_categories(meta, glean);
        match label {
            ...
            DynamicLabelType::KeyOnly(_) => {
                if key_count >= MAX_LABELS || !label_is_valid(key, glean, meta) {
                    key = OTHER_LABEL;
                }
            }
            DynamicLabelType::CategoryOnly(_) => {
                if category_count >= MAX_LABELS || !label_is_valid(category, glean, meta) {
                    category = OTHER_LABEL;
                }
            }
...
fn count_keys_and_categories(meta: &CommonMetricDataInternal, glean: &Glean) -> (usize, usize) {
    let base_identifier = &meta.base_identifier();
    let prefix = format!("{base_identifier}{RECORD_SEPARATOR}");
    let mut seen_keys: Vec<String> = Vec::new();
    let mut seen_categorires: Vec<String> = Vec::new();
    let mut snapshotter = |mut metric_id: &[u8], _: &Metric| {
        let mut metric_id_str = String::new();
        _ = metric_id.read_to_string(&mut metric_id_str);

        // Split full identifier on the ASCII Record Separator (\x1e)
        let parts: Vec<&str> = metric_id_str.split(RECORD_SEPARATOR).collect();

        if parts.len() == 2 {
            if !seen_keys.contains(&parts[0].into()) {
                seen_keys.push(parts[0].into());
            }
            if !seen_categorires.contains(&parts[1].into()) {
                seen_categorires.push(parts[1].into());
            }
        } else {
            println!("DualLabeledCounter label does not contain exactly 2 parts");
        }
    };
    ...
    (seen_keys.len(), seen_categorires.len())
}
