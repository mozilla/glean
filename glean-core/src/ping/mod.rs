...
impl PingMaker {
    ...
    fn get_client_info(&self, glean: &Glean, include_client_id: bool, included_info_sections: &[String]) -> JsonValue {
        ...
        if !included_info_sections.is_empty() {
            ...
            obj.retain(|key, _| {
                included_info_sections.contains(key)
            });
        }
    ...
    pub fn collect<'a>(
        ...
        let mut json = if ping.include_info_sections() {
            ...
            let client_info = self.get_client_info(glean, ping.include_client_id(), ping.included_info_sections());
