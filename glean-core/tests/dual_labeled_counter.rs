...
use glean_core::{CommonMetricData, LabeledMetricData, Lifetime};
...
fn dynamic_labels_regex_mismatch() {
    ...
    let metric = dual_labeled_counter.get("non-ASCII-key�", "non-ASCII-cat�");
    metric.add_sync(&glean, 1);
    ...
    // We end up with 2x InvalidLabel errors, one each for key and category which both contain
    // invalid characters.
    assert_eq!(
        json!({
            "dual_labeled_counter": {
