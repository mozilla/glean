...
fn overrun_the_label_count_with_a_single_label() {
    ...
    let pings = (0..16).map(|i| {
        new_test_ping(&mut glean, &format!("test-ping-{i}"))
    }).collect::<Vec<_>>();

    let send_in_pings = pings.iter().map(|p| p.name().to_string()).collect::<Vec<_>>();
    ...
    assert!(test_get_num_recorded_errors(&glean, labeled.submetric.meta(), ErrorType::InvalidLabel).is_err());
    assert_eq!(1, metric1.get_value(&glean, None).unwrap());

    let metric2 = labeled.get("label-2");
    metric2.add_sync(&glean, 23);
    assert!(test_get_num_recorded_errors(&glean, metric2.meta(), ErrorType::InvalidLabel).is_err());
