...
fn set_up_basic_ping() -> (Glean, PingMaker, PingType, tempfile::TempDir) {
    ...
    let ping_type = PingType::new("store1", true, false, true, true, true, vec![]);
...
fn experimentation_id_is_removed_if_send_if_empty_is_false() {
    ...
    let unknown_ping_type = PingType::new("unknown", true, false, true, true, true, vec![]);
...
fn seq_number_must_be_sequential() {
    ...
    for i in 0..=1 {
        for ping_name in ["store1", "store2"].iter() {
            let ping_type = PingType::new(*ping_name, true, false, true, true, true, vec![]);
            let ping = ping_maker
                .collect(&glean, &ping_type, None, "", "")
                .unwrap();
    ...
    {
        let ping_type = PingType::new("store2", true, false, true, true, true, vec![]);
...
fn no_pings_submitted_if_upload_disabled() {
    ...
    let (mut glean, _t) = new_glean(None);
    let ping_type = PingType::new("store1", true, true, true, true, true, vec![]);
    glean.register_ping_type(&ping_type);
